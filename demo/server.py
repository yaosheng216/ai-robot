...
class Master:
    def __init__(self):
        ...
        self.prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    self.SYSTEMPL.format(who_you_are=self.MOODS[self.STATUS]["roleSet"]),
                ),
    ...
    def get_memory(self):
        ...
        if len(store_message) > 10:
            ...
            summary = chain.invoke({"input": store_message, "who_you_are": self.MOODS[self.STATUS]["roleSet"]})
    ...
    async def get_voice(self, text: str, uid: str):
        ...
        body = f"""<speak version='1.0' xmlns='http://www.w3.org/2001/10/synthesis' xmlns:mstts="https://www.w3.org/2001/mstts" xml:lang='zh-CN'>
            <voice name='zh-CN-YunzeNeural'>
                <mstts:express-as style="{self.MOODS.get(str(self.STATUS), {"voiceStyle": "default"})["voiceStyle"]}" role="SeniorMale">{text}</mstts:express-as>
            </voice>
        </speak>"""
        # 发送请求
